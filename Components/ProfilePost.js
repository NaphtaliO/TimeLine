...
const ProfilePost = ({ navigation, item }) => {
    ...
    return (
        <>
            ...
            <View style={{ flexDirection: 'row', marginBottom: 10, marginLeft: 'auto', marginRight: 7 }}>
                <TouchableWithoutFeedback onPress={() => { navigation.navigate("CommentsScreen", { post_id: item._id }) }}><Text style={{ paddingRight: 5 }}>{comments.length} comments</Text></TouchableWithoutFeedback>
                <Text style={{ paddingRight: 5 }}>{likes.length} likes</Text>
            </View>
            <ItemSeparator />
